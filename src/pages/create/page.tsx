...
import { ArrowRight, Send, MapPin, Tag, LogIn } from "lucide-react";
...
function CreateListingPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (priceType === "fixed" && !price) { toast.error("يرجى إدخال السعر"); return; }
    if (priceType === "auction" && !startingBid) { toast.error("يرجى إدخال سعر البداية"); return; }
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.3, ease: "easeOut" }}>
          <MediaUploader images={images} onImagesChange={setImages} />
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.3, ease: "easeOut" }} className="space-y-2">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.3, ease: "easeOut" }} className="space-y-2">
          <Label htmlFor="description" className="text-sm font-bold">الوصف</Label>
          <Textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="اكتب وصفاً تفصيلياً للإعلان..." className="rounded-xl min-h-[100px] resize-none" dir="rtl" />
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.3, ease: "easeOut" }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label className="text-sm font-bold flex items-center gap-1.5">
              <Tag className="h-3.5 w-3.5 text-muted-foreground" />النوع
            </Label>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.3, ease: "easeOut" }}>
          <PriceTypeSelector type={priceType} onTypeChange={setPriceType} />
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.3, ease: "easeOut" }} className="space-y-2">
          {priceType === "fixed" ? (
            <>
            ...
            <>
              ...
              <Input id="startingBid" type="number" value={startingBid} onChange={(e) => setStartingBid(e.target.value)} placeholder="0" className="rounded-xl h-11 text-lg font-bold tabular-nums" dir="ltr" />
              <p className="text-[10px] text-muted-foreground">المزاد يبدأ من هذا السعر ويمكن للمشترين المزايدة عليه</p>
            </>
          )}
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.3, ease: "easeOut" }} className="space-y-3 pb-8">
