...
import { Radio, TrendingUp, Search } from "lucide-react";
...
import { useLiveStreams } from "@/hooks/use-mock-data.ts";
...
export default function Index() {
  ...
  const streams = useLiveStreams(category);
        ...
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 bg-red-600/10 text-red-500 rounded-full px-3 py-1.5">
              ...
              <span className="text-xs font-bold">{streams?.filter((s) => s.isLive).length} بث مباشر</span>
            </div>
          </div>
        </div>
        <CategoryFilter
